...
function onCellPrepared(e) {
    if (e.rowType === "data" && e.column.dataField === "abWriteOff") {
        var value = e.value;
        var checkbox = $("<input type='checkbox' disabled='disabled' />");
        if (value === true) {
            checkbox.prop("checked", true);
        }
        $(e.cellElement).empty().append(checkbox);
    }
}
...
function booleanCellTemplate(container, options) {
    var value = options.value;
    var checkBox = $("<input type='checkbox' disabled />")
        .prop("checked", value)
        .prop("indeterminate", value === null)
        .appendTo(container);
}



function setWriteOff(newData, value, currentRowData) {
    jQuery.ajax({
        type: "POST",
        url: `UpdateRow?table=tAsset&&field=abWriteOff&&type=bool&&data=${value}&&anQId=${currentRowData.anQId}`,
        dataType: "json",
...
function setWriteOffReason(newData, value, currentRowData) {
    jQuery.ajax({
        type: "POST",
        url: `UpdateRow?table=tAsset&&field=abWriteOffReason&&type=string&&data=${value}&&anQId=${currentRowData.anQId}`,
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (response) {
            window.location.reload();
